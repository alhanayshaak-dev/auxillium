...
export default function AskLezPage() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/lez-chat', {
        method: 'POST',
        headers: {
    ...
    setChatSessions(prev => prev.map(session => 
      session.id === currentSession 
        ? { 
            ...
            lastMessage: action,
            timestamp: userMessage.timestamp,
            title: session.title === 'New Chat' ? action.substring(0, 30) + '...' : session.title
          }
        : session
    ))
      ...
      <div className="bg-gradient-to-r from-red-50 to-pink-50 dark:from-red-900/30 dark:to-pink-900/30 px-4 py-6 flex-shrink-0">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <ImmersiveReader 
              content="Ask Lez - Your AI Health Assistant. Lez can help answer health questions, guide you through Auxillium app features, assist with medical inquiries, provide health information, and help you navigate healthcare services. Ask about symptoms, medications, appointments, or any health-related concerns."
              title="Ask Lez - AI Health Assistant"
            />
            <button 
              onClick={() => router.back()}
